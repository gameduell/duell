...
class Type:
    _hx_class_name = "Type"
    __slots__ = ()
    _hx_statics = ["getClass", "getEnum", "getSuperClass", "getClassName", "getEnumName", "resolveClass", "resolveEnum", "createEmptyInstance", "createEnum", "getEnumConstructs", "typeof"]
    ...
    def run(self):
        try:
            ...
            if isMissingSelfSetup:
                ...
                if (duell_objects_Arguments.getSelectedCommand().name != "run"):
                    duell_helpers_LogHelper.println(((" Time passed " + Std.string(((((python_lib_Time.mktime(Date.now().date.timetuple()) * 1000) - currentTime)) / 1000))) + HxOverrides.stringOrNull((((" sec for command \"" + HxOverrides.stringOrNull(duell_objects_Arguments.getSelectedCommand().name)) + "\"")))))
        except Exception as _hx_e:
            _hx_e1 = _hx_e.val if isinstance(_hx_e, _HxException) else _hx_e
            error = _hx_e1
...
class duell_commands_BuildCommand:
    ...
    __slots__ = ("libList", "buildLib", "buildGitVers", "platformName", "duellConfig", "currentXMLPath")
    ...
    def determinePlatformToBuildFromArguments(self):
        ...
        if (platformNameCorrectnessCheck.matchObj is None):
    ...
    def checkUpdateTime(self):
        ...
        previousHours = ((((python_lib_Time.mktime(previousDate.date.timetuple()) * 1000) / 1000.0) / 60.0) / 60.0)
        currentHours = ((((python_lib_Time.mktime(currentDate.date.timetuple()) * 1000) / 1000.0) / 60.0) / 60.0)
    ...
    def suggestUpdate(self):
        ...
        if answer:
            ...
            updateArguments = list(filter(_hx_local_0,duell_objects_Arguments.getRawArguments()))
            ...
            duell_helpers_CommandHelper.runHaxelib(Sys.getCwd(),(["run", "duell_duell"] + duell_objects_Arguments.getRawArguments()),_hx_AnonObject({}))
            Sys.exit(0)
        else:
            tmp = Sys.getCwd()
            self.duellConfig.lastProjectFile = haxe_io_Path.join([tmp, duell_defines_DuellDefines.PROJECT_CONFIG_FILENAME])
    ...
    def buildNewExecutableWithBuildLibAndDependencies(self):
        ...
        while (_g < len(_g1)):
            ...
            if sys_FileSystem.exists(haxe_io_Path.join([refPath, "duell", "build", "plugin", "library", l.lib.name, "LibraryXMLParser.hx"])):
                x3 = (("duell.build.plugin.library." + HxOverrides.stringOrNull(l.lib.name)) + ".LibraryXMLParser")
            ...
            if sys_FileSystem.exists(haxe_io_Path.join([refPath, "duell", "build", "plugin", "library", l.lib.name, "LibraryBuild.hx"])):
                x4 = (("duell.build.plugin.library." + HxOverrides.stringOrNull(l.lib.name)) + ".LibraryBuild")
        ...
        x5 = ("platform_" + HxOverrides.stringOrNull(self.platformName))
        buildArguments.append(x5)
        buildArguments.append("-D")
        x6 = ("host_" + HxOverrides.stringOrNull(duell_helpers_PlatformHelper.get_hostPlatform().tag.lower()))
        buildArguments.append(x6)
        buildArguments.append("-D")
        x7 = ("duell_api_level=" + Std.string(duell_defines_DuellDefines.DUELL_API_LEVEL))
        buildArguments.append(x7)
        buildArguments.append("-resource")
        x8 = (HxOverrides.stringOrNull(haxe_io_Path.join([duell_helpers_DuellLibHelper.getPath("duell"), "config.xml"])) + "@generalArguments")
        buildArguments.append(x8)
        ...
        runArguments = (runArguments + duell_objects_Arguments.getRawArguments())
    ...
    def runFast(self):
        ...
        result = duell_helpers_CommandHelper.runCommand("",python_lib_Sys.executable,runArguments,_hx_AnonObject({'errorMessage': "running the plugin", 'exitOnError': False}))
        if (result != 0):
            Sys.exit(result)
    ...
    def parseXML(self,path):
        ...
        _this.append(x)
        element = xml.get_elements()
        while element.hasNext():
            element1 = element.next()
            _g = element1.get_name()
            _g1 = _g
            _hx_local_0 = len(_g1)
            if (_hx_local_0 == 9):
                if (_g1 == "sourcelib"):
                    if (element1.has.resolve("path") and element1.has.resolve("name")):
                        includePath1 = self.resolvePath(element1.att.resolve("path"))
                        newSourceLib = duell_objects_SourceLib(element1.att.resolve("name"),includePath1)
                        self.handleDuellLibParsed(newSourceLib)
            elif (_hx_local_0 == 7):
                if (_g1 == "include"):
                    if element1.has.resolve("path"):
                        includePath = self.resolvePath(element1.att.resolve("path"))
                        self.parseXML(includePath)
            elif (_hx_local_0 == 8):
                if (_g1 == "duelllib"):
                    name = None
                    version = None
                    if ((not element1.has.resolve("version")) or ((element1.att.resolve("version") == ""))):
                        raise _HxException(("DuellLib dependencies must always specify a version. File: " + ("null" if path is None else path)))
                    name = element1.att.resolve("name")
                    version = element1.att.resolve("version")
...
class duell_commands_CreateCommand:
    ...
    def determineDuellLibraryFromArguments(self):
        ...
        if (pluginNameCorrectnessCheck.matchObj is None):
...
class duell_commands_DependencyCommand:
    ...
    def buildVisualization(self,creator):
        ...
        executableFile = ("dot.exe" if ((duell_helpers_PlatformHelper.get_hostPlatform() == duell_helpers_Platform.WINDOWS)) else "dot")
...
class duell_commands_EnvironmentSetupCommand:
    ...
    __slots__ = ("setupLib", "platformName", "gitvers")
    ...
    def buildNewEnvironmentWithSetupLib(self):
        ...
        duellConfig = duell_objects_DuellConfigJSON.getConfig(duell_helpers_DuellConfigHelper.getDuellConfigFileLocation())
        def _hx_local_0(s):
            return (not StringTools.startsWith(HxOverrides.arrayGet(s.split(":"), 0),_gthis.platformName))
        duellConfig.setupsCompleted = list(filter(_hx_local_0,duellConfig.setupsCompleted))
        _this = duellConfig.setupsCompleted
        x3 = ((HxOverrides.stringOrNull(self.platformName) + ":") + HxOverrides.stringOrNull(self.gitvers.currentVersion))
        _this.append(x3)
...
class duell_commands_RepoConfigCommand:
    ...
    def execute(self):
        ...
        if duell_objects_Arguments.isSet("-addFront"):
            ...
            x = duell_commands_RepoConfigCommand.defaultRepoListURL
            _this2.append(x)
            duellConfig.writeToConfig()
    ...
    def printDuplicates(self):
        ...
        while repo.hasNext():
            repo1 = repo.next()
            duell_helpers_LogHelper.info(("Duplicated key: " + ("null" if repo1 is None else repo1)))
...
class duell_commands_RunCommand:
    ...
    __slots__ = ("runLib", "buildGitVers", "pluginName", "pluginSourceLibPath", "duellConfig")
    ...
    def buildNewRunExecutableWithRunLib(self):
        ...
        buildArguments.append(x1)
        buildArguments.append("-D")
        x2 = ("run_plugin_" + HxOverrides.stringOrNull(self.pluginName))
        buildArguments.append(x2)
        buildArguments.append("-D")
        buildArguments.append("plugin")
        buildArguments.append("-resource")
...
class duell_commands_ToolSetupCommand:
    ...
    def setupHaxe(self):
        ...
        systemCommand = (False if (((haxePath is not None) and ((haxePath != "")))) else True)
    ...
    def setupDuellSettingsDirectory(self):
        ...
        if (python_internal_ArrayImpl.indexOf(duellConfig.repoListURLs,repoListURL,None) == -1):
            if (len(duellConfig.repoListURLs) > 0):
                _this = duellConfig.repoListURLs
                duell_helpers_LogHelper.println((("There are already a repo list urls configured (" + HxOverrides.stringOrNull(",".join([python_Boot.toString1(x1,'') for x1 in _this]))) + ")"))
    ...
    def installDuell(self):
        ...
        duellConfig = duell_objects_DuellConfigJSON.getConfig(duell_helpers_DuellConfigHelper.getDuellConfigFileLocation())
        _this = duellConfig.repoListURLs
        duell_helpers_LogHelper.println(("Will install the duell tool from " + HxOverrides.stringOrNull(", ".join([python_Boot.toString1(x1,'') for x1 in _this]))))
        libMap = duell_helpers_DuellLibListHelper.getDuellLibReferenceList()
        if (not ("duell" in libMap.h)):
            _this1 = duellConfig.repoListURLs
            raise _HxException(("Could not find \"duell\" in " + HxOverrides.stringOrNull(", ".join([python_Boot.toString1(x1,'') for x1 in _this1]))))
    ...
    def writeHXCPPDefinitions(self):
        if (duell_helpers_PlatformHelper.get_hostPlatform() == duell_helpers_Platform.MAC):
            ...
            _hx_def = newDefines.keys()
            while _hx_def.hasNext():
                def1 = _hx_def.next()
                duell_helpers_LogHelper.println(((("\x1B[1m" + ("null" if def1 is None else def1)) + "\x1B[0m:") + HxOverrides.stringOrNull(newDefines.h.get(def1,None))))
            def2 = existingDefines.keys()
            while def2.hasNext():
                def3 = def2.next()
                if (not (def3 in newDefines.h)):
                    value = existingDefines.h.get(def3,None)
                    newDefines.h[def3] = value
...
class duell_commands_UpdateCommand:
    ...
    __slots__ = ("finalLibList", "finalPluginList", "finalToolList", "haxelibVersions", "duellLibVersions", "sourceLibVersions", "pluginVersions", "buildLib", "platformName", "projectDir", "duellToolGitVers", "duellToolRequestedVersion", "isDifferentDuellToolVersion", "targetPlatform", "currentXMLPath")
    ...
    def execute(self):
        ...
        self.targetPlatform = (duell_objects_Arguments.get("-target") if (duell_objects_Arguments.isSet("-target")) else None)
    ...
    def synchronizeRemotes(self):
        ...
        while key.hasNext():
            key1 = key.next()
            libRef = reflist.h.get(key1,None)
            duellLib = duell_objects_DuellLib.getDuellLib(key1)
    ...
    def iterateDuellLibVersionsUntilEverythingIsParsedAndVersioned(self):
        while True:
            ...
            _g = []
            l = self.duellLibVersions.iterator()
            while l.hasNext():
                l1 = l.next()
                _g.append(l1)
            duelllibClone = _g
            ...
            while (_g1 < len(duelllibClone)):
                ...
                if (_g21 == 0):
                    ...
                    pass
            _g11 = []
            l2 = self.sourceLibVersions.iterator()
            while l2.hasNext():
                l3 = l2.next()
                _g11.append(l3)
            sourcelibClone = _g11
            _g22 = 0
            while (_g22 < len(sourcelibClone)):
                sourceLibVersion = (sourcelibClone[_g22] if _g22 >= 0 and _g22 < len(sourcelibClone) else None)
                _g22 = (_g22 + 1)
    ...
    def checkVersionsOfPlugins(self):
        pluginVersion = self.pluginVersions.iterator()
        while pluginVersion.hasNext():
            pluginVersion1 = pluginVersion.next()
            duell_helpers_LogHelper.info("\n")
            duell_helpers_LogHelper.info(((("checking version of " + "\x1B[1m") + HxOverrides.stringOrNull(pluginVersion1.lib.name)) + "\x1B[0m"))
            resolvedVersion = pluginVersion1.gitVers.solveVersion(pluginVersion1.lib.version,duell_objects_Arguments.isSet("-rc"),duell_objects_Arguments.get("-overridebranch"))
            if pluginVersion1.gitVers.needsToChangeVersion(resolvedVersion):
                duell_helpers_LogHelper.info(((("  - changing to version " + "\x1B[1m") + ("null" if resolvedVersion is None else resolvedVersion)) + "\x1B[0m"))
                pluginVersion1.gitVers.changeToVersion(resolvedVersion)
            else:
                duell_helpers_LogHelper.info(((("  - using version " + "\x1B[1m") + HxOverrides.stringOrNull(pluginVersion1.gitVers.currentVersion)) + "\x1B[0m"))
    ...
    def checkDuellToolVersion(self):
        ...
        x = _hx_AnonObject({'name': "duell tool", 'version': self.duellToolGitVers.currentVersion})
        _this.append(x)
    ...
    def printFinalResult(self,duellLibs,sourceLibs,haxelibs,plugins):
        ...
        while (_g1 < len(sourceLibs)):
            ...
            _this = lib1.getPath()
            _this1 = (list(_this) if ((negativePath == "")) else _this.split(negativePath))
            parentPath = (None if ((len(_this1) == 0)) else _this1.pop())
        ...
        if ((lockedDiffs is not None) and ((len(lockedDiffs) != 0))):
            ...
            while (_g < len(lockedDiffs)):
                ...
                if (not (lockedDiff.typeReadable in outputFilter.h)):
                    key = lockedDiff.typeReadable
                    value = list()
                    outputFilter.h[key] = value
                _this = outputFilter.h.get(lockedDiff.typeReadable,None)
                _this.append(lockedDiff)
            key1 = outputFilter.keys()
            while key1.hasNext():
                key2 = key1.next()
                duell_helpers_LogHelper.info((((("\x1B[1m" + "   ") + ("null" if key2 is None else key2)) + ":") + "\x1B[0m"))
                diffs = outputFilter.h.get(key2,None)
                _g1 = 0
                while (_g1 < len(diffs)):
    ...
    def createFinalLibLists(self):
        duellLibVersion = self.duellLibVersions.iterator()
        while duellLibVersion.hasNext():
            duellLibVersion1 = duellLibVersion.next()
            _this = self.finalLibList.duellLibs
            x = duell_objects_DuellLib.getDuellLib(duellLibVersion1.name,duellLibVersion1.gitVers.currentVersion)
            _this.append(x)
        sourceLibVersion = self.sourceLibVersions.iterator()
        while sourceLibVersion.hasNext():
            sourceLibVersion1 = sourceLibVersion.next()
            _this1 = self.finalLibList.sourceLibs
            x1 = duell_objects_SourceLib(sourceLibVersion1.name,sourceLibVersion1.path)
            _this1.append(x1)
        ...
        haxelibVersion = self.haxelibVersions.iterator()
        while haxelibVersion.hasNext():
            haxelibVersion1 = haxelibVersion.next()
            _this2 = self.finalLibList.haxelibs
            _this2.append(haxelibVersion1)
        self.finalLibList.haxelibs.sort(key= python_lib_Functools.cmp_to_key(self.sortHaxeLibsByName))
        plugin = self.pluginVersions.keys()
        while plugin.hasNext():
            plugin1 = plugin.next()
            _this3 = self.finalPluginList
            x2 = duell_objects_DuellLib.getDuellLib(self.pluginVersions.h.get(plugin1,None).lib.name,self.pluginVersions.h.get(plugin1,None).gitVers.currentVersion)
            _this3.append(x2)
    ...
    def createSchemaXml(self,duellLibList,pluginLibList,sourceLibList):
        ...
        while (_g3 < len(sourceLibList)):
            ...
            _g3 = (_g3 + 1)
            _g21.append(s)
        duell_helpers_SchemaHelper.createSchemaXml(tmp,tmp1,_g21)
    ...
    def parseXML(self,path,sourceLibrary = "project"):
        ...
        while element.hasNext():
            element1 = element.next()
            if ((self.targetPlatform is not None) and (not duell_helpers_XMLHelper.isValidElement(element1,[self.targetPlatform]))):
                continue
            _g = element1.get_name()
            _g1 = _g
            _hx_local_0 = len(_g1)
            if (_hx_local_0 == 22):
                if (_g1 == "supported-build-plugin"):
                    name2 = None
                    version2 = None
                    if ((not element1.has.resolve("version")) or ((element1.att.resolve("version") == ""))):
                        raise _HxException(("Supported builds must always specify a version. File: " + ("null" if path is None else path)))
                    name2 = element1.att.resolve("name")
                    version2 = element1.att.resolve("version")
                ...
                if (_g1 == "haxelib"):
                    name1 = element1.att.resolve("name")
                    if (name1 == ""):
                        continue
                    version1 = None
                    if ((not element1.has.resolve("version")) or ((element1.att.resolve("version") == ""))):
                        ...
                        version1 = element1.att.resolve("version")
                    haxelib = duell_objects_Haxelib.getHaxelib(name1,version1)
                ...
                if (_g1 == "supported-duell-tool"):
                    version3 = None
                    version3 = element1.att.resolve("version")
                    ...
                    if (self.duellToolRequestedVersion is None):
                        ...
                        try:
                            tmp = self.duellToolGitVers
                            tmp1 = [version3, self.duellToolRequestedVersion.version]
                            tmp2 = duell_objects_Arguments.get("-overridebranch")
                            self.duellToolRequestedVersion.version = tmp.resolveVersionConflict(tmp1,tmp2,None,["duell tool", sourceLibrary, self.duellToolRequestedVersion.dependTo])
                        except Exception as _hx_e:
                            _hx_e1 = _hx_e.val if isinstance(_hx_e, _HxException) else _hx_e
    ...
    def handleSourceLibParsed(self,newSourceLib):
        sourceLibName = self.sourceLibVersions.keys()
        while sourceLibName.hasNext():
            sourceLibName1 = sourceLibName.next()
            if (sourceLibName1 != newSourceLib.name):
                continue
            sourceLibVersion = self.sourceLibVersions.h.get(newSourceLib.name,None)
            sourceLibVersion.versionState = duell_commands_VersionState.ParsedVersionUnchanged
            return
        this1 = self.sourceLibVersions
        k = newSourceLib.name
    ...
    def handleDuellLibParsed(self,newDuellLib,sourceLibrary):
        ...
        duellLibName = self.duellLibVersions.keys()
        while duellLibName.hasNext():
            duellLibName1 = duellLibName.next()
            if (duellLibName1 != newDuellLib.name):
                continue
    ...
    def userFileHasDuellNamespace():
        userFile = duell_helpers_DuellConfigHelper.getDuellUserFileLocation()
        if sys_FileSystem.exists(userFile):
            return duell_helpers_SchemaHelper.hasDuellNamespace(userFile)
        else:
            return False
...
class duell_helpers_CommandHelper:
    ...
    def runCommand(path,command,args,options = None):
        ...
        systemCommand = (Reflect.field(options,"systemCommand") if (((options is not None) and ((Reflect.field(options,"systemCommand") is not None)))) else True)
        logOnlyIfVerbose = (Reflect.field(options,"logOnlyIfVerbose") if (((options is not None) and ((Reflect.field(options,"logOnlyIfVerbose") is not None)))) else True)
        exitOnError = (Reflect.field(options,"exitOnError") if (((options is not None) and ((Reflect.field(options,"exitOnError") is not None)))) else True)
        errorMessage = (Reflect.field(options,"errorMessage") if (((options is not None) and ((Reflect.field(options,"errorMessage") is not None)))) else None)
        nonErrorExitCodes = (Reflect.field(options,"nonErrorExitCodes") if (((options is not None) and ((Reflect.field(options,"nonErrorExitCodes") is not None)))) else [0])
        ...
        if ((python_internal_ArrayImpl.indexOf(nonErrorExitCodes,result,None) == -1) and exitOnError):
            pathString = ((" - in path: " + ("null" if path is None else path)) if (((path is not None) and ((path != "")))) else "")
            additionalMessage = ((" - Action was: " + ("null" if errorMessage is None else errorMessage)) if ((errorMessage is not None)) else "")
...
class duell_helpers_DuellConfigHelper:
    ...
    def checkIfDuellConfigExists():
        if sys_FileSystem.exists(duell_helpers_DuellConfigHelper.getDuellConfigFolderLocation()):
            return sys_FileSystem.exists(duell_helpers_DuellConfigHelper.getDuellConfigFileLocation())
        else:
            return False
    ...
    def getPath(name):
        ...
        while (_g1 < _g):
            ...
            if StringTools.startsWith(StringTools.trim((lines[i] if i >= 0 and i < len(lines) else None)),"-D"):
                this1 = duell_helpers_DuellLibHelper.caches.pathCache
    ...
    def addLibsToTheRepoCache(configJSON):
        ...
        while (_g < len(listOfRepos)):
            ...
            if (repo in duell_helpers_DuellLibListHelper.repoListCache.h):
                duplicates = True
            this1 = duell_helpers_DuellLibListHelper.repoListCache
...
class duell_helpers_FileHelper:
        ...
        while (_g < len(files)):
            ...
            if ((file != ".") and ((file != ".."))):
                ...
                if sys_FileSystem.isDirectory(itemSource):
                    retFiles = (retFiles + duell_helpers_FileHelper.getAllFilesInDir(itemSource))
                else:
    ...
    def recursiveCopyFiles(source,destination,onlyIfNewer = True,purgeDestination = False):
        ...
        if (duell_helpers_PlatformHelper.get_hostPlatform() == duell_helpers_Platform.WINDOWS):
            purgeArg = ("/MIR" if purgeDestination else "/E")
    ...
    def listBranches(destination):
        ...
        while (_g < len(outputList)):
            ...
            if StringTools.startsWith(line,"remotes/"):
                ...
                remote = HxString.substr(line,0,line.find("/"))
                branch = HxString.substr(line,(line.find("/") + 1),None)
...
class duell_helpers_ParallelProcessHelper:
    ...
    def run(self,jobs):
        ...
        while (_g1 < _g):
            ...
            args = (tuple(Reflect.field(job,"args")) if (hasattr(job,(("_hx_" + "args") if (("args" in python_Boot.keywords)) else (("_hx_" + "args") if (((((len("args") > 2) and ((ord("args"[0]) == 95))) and ((ord("args"[1]) == 95))) and ((ord("args"[(len("args") - 1)]) != 95)))) else "args")))) else None)
    ...
    def mkdir(directory):
        ...
        if ((duell_helpers_PlatformHelper.get_hostPlatform() != duell_helpers_Platform.WINDOWS) and (((parts[0] if 0 < len(parts) else None) == "~"))):
            ...
            if (len(parts) != 0):
                parts.pop(0)
        tmp = None
        if ((duell_helpers_PlatformHelper.get_hostPlatform() == duell_helpers_Platform.WINDOWS) and ((len((parts[0] if 0 < len(parts) else None)) == 2))):
            _this = (parts[0] if 0 < len(parts) else None)
            tmp = ((("" if ((1 >= len(_this))) else _this[1])) == ":")
        else:
            tmp = False
        if tmp:
...
class duell_helpers_PlatformHelper:
    ...
    def get_macVersion():
        ...
        if (duell_helpers_PlatformHelper._macVersion is None):
            ...
            versions = list(map(_hx_local_0,output.split(".")))
        ...
        if (sourcelibs is not None):
            ...
            while (_g1 < len(sourcelibs)):
                ...
                if sys_FileSystem.exists(sourceLibSchemaPath):
                    x = _hx_AnonObject({'name': sourcelib.name, 'path': sourceLibSchemaPath})
                    librariesWithSchema.append(x)
                else:
                    x1 = sourcelib.name
                    librariesWithoutSchema.append(x1)
        _g2 = 0
...
class duell_helpers_Template:
    ...
    __slots__ = ("expr", "context", "foreach_index_stack", "macros", "stack", "buf")
    ...
    def parseExpr(self,data):
        ...
        while True:
            ...
            p = duell_helpers_Template.expr_splitter.matchObj.group(0)
    ...
    def run(self,e):
        ...
        if (e1 == 0):
            ...
            v3 = v2
            index = 0
            _this3 = self.foreach_index_stack
            _this3.append(index)
            ctx = v3
            while ctx.hasNext():
                ctx1 = ctx.next()
...
class duell_helpers_XMLHelper:
    ...
    def isValidElement(element,validatingConditions):
        def _hx_local_7(value):
            ...
            optionalDefines = (value.split("||") if ((value.find("||") != -1)) else value.split("[or]"))
            def _hx_local_0(_hx_str):
                return StringTools.trim(_hx_str)
...
class duell_objects_ArgumentSpec_Int:
    ...
    __slots__ = ("name", "documentation", "set", "value")
...
class duell_objects_CommandSpec:
    ...
    __slots__ = ("name", "hasPlugin", "documentation", "commandHandler", "arguments", "configurationDocumentation")
...
class duell_objects_Arguments:
    ...
    def validateArguments():
        ...
        if duell_objects_Arguments.selectedCommand.hasPlugin:
            ...
            if (len(args) != 1):
                ...
                tmp = ((("" if ((0 >= len(_this1))) else _this1[0])) == "-")
            else:
                tmp = True
            if tmp:
        ...
        while (index < len(args)):
            argString = index
            index = (index + 1)
            argString1 = (args[argString] if argString >= 0 and argString < len(args) else None)
            if (argString1 == "-D"):
                args1 = index
                index = (index + 1)
                duell_objects_Arguments.parseDefine((args[args1] if args1 >= 0 and args1 < len(args) else None))
                continue
            argSpec = None
            if ((duell_objects_Arguments.generalArgumentSpecs is not None) and (argString1 in duell_objects_Arguments.generalArgumentSpecs.h)):
                argSpec = duell_objects_Arguments.generalArgumentSpecs.h.get(argString1,None)
            elif ((duell_objects_Arguments.selectedCommand.arguments is not None) and (argString1 in duell_objects_Arguments.selectedCommand.arguments.h)):
                argSpec = duell_objects_Arguments.selectedCommand.arguments.h.get(argString1,None)
            elif ((duell_objects_Arguments.pluginArgumentSpecs is not None) and (argString1 in duell_objects_Arguments.pluginArgumentSpecs.h)):
                argSpec = duell_objects_Arguments.pluginArgumentSpecs.h.get(argString1,None)
            if (argSpec is None):
                Sys.println((("Unknown argument \"" + ("null" if argString1 is None else argString1)) + "\""))
                return False
            else:
                Reflect.setField(argSpec,"set",True)
                if (Type.getClass(argSpec) != Type.getClass(argSpecNoArgumentValue)):
                    if (Type.getClass(argSpec) == Type.getClass(argSpecInt)):
                        if (len(args) == index):
                            Sys.println((("Argument " + ("null" if argString1 is None else argString1)) + " expected an int, but got nothing"))
                            return False
                        else:
                            argParam = index
                            index = (index + 1)
                            argParam1 = (args[argParam] if argParam >= 0 and argParam < len(args) else None)
                            argParamInt = Std.parseInt(argParam1)
                            if (argParamInt is None):
                                Sys.println(((("Argument " + ("null" if argString1 is None else argString1)) + " expected an int, but got ") + ("null" if argParam1 is None else argParam1)))
                                return False
                            else:
                        ...
                        argParam2 = index
                        index = (index + 1)
                        argParam3 = (args[argParam2] if argParam2 >= 0 and argParam2 < len(args) else None)
                        Reflect.setField(argSpec,"value",argParam3)
                    else:
    ...
    def parseConfig():
        ...
        while element.hasNext():
            ...
            _hx_local_0 = len(_g1)
            if (_hx_local_0 == 13):
                if (_g1 == "documentation"):
                    duell_objects_Arguments.generalDocumentation = StringTools.htmlUnescape(element1.get_innerData())
            elif (_hx_local_0 == 3):
                if (_g1 == "arg"):
                    argSpec = duell_objects_Arguments.parseArgumentSpec(element1)
                    duell_objects_Arguments.generalArgumentSpecs.set(Reflect.field(argSpec,"name"),argSpec)
            elif (_hx_local_0 == 7):
                if (_g1 == "command"):
                    commandSpec = duell_objects_Arguments.parseCommandSpec(element1)
                    duell_objects_Arguments.commandSpecs.set(commandSpec.name,commandSpec)
            else:
                pass
    ...
    def parsePlugin(fileString):
        ...
        while element.hasNext():
            ...
            _hx_local_0 = len(_g1)
            if (_hx_local_0 == 13):
                if (_g1 == "configuration"):
                    configurationElements = element1.get_elements()
                    configElem = configurationElements
                    while configElem.hasNext():
                        configElem1 = configElem.next()
                        ...
                        name = configElem1.att.resolve("name")
                        documentation = StringTools.htmlUnescape(StringTools.trim(configElem1.get_innerData()))
                        duell_objects_Arguments.pluginConfigurationDocumentation.set(name,documentation)
                elif (_g1 == "documentation"):
                    duell_objects_Arguments.pluginDocumentation = StringTools.htmlUnescape(StringTools.trim(element1.get_innerData()))
            elif (_hx_local_0 == 3):
                if (_g1 == "arg"):
                    if (duell_objects_Arguments.pluginArgumentSpecs is None):
                        duell_objects_Arguments.pluginArgumentSpecs = haxe_ds_StringMap()
                    argSpec = duell_objects_Arguments.parseArgumentSpec(element1)
                    duell_objects_Arguments.pluginArgumentSpecs.set(Reflect.field(argSpec,"name"),argSpec)
            else:
                pass
    ...
    def parseCommandSpec(command):
        ...
        if command.hasNode.resolve("args"):
            argXML = command.node.resolve("args").get_elements()
            while argXML.hasNext():
                argXML1 = argXML.next()
                if (args is None):
                    args = haxe_ds_StringMap()
                argSpec = duell_objects_Arguments.parseArgumentSpec(argXML1)
                args.h[Reflect.field(argSpec,"name")] = argSpec
        configuration = None
        if command.hasNode.resolve("configuration"):
            argXML2 = command.node.resolve("configuration").get_elements()
            while argXML2.hasNext():
                argXML3 = argXML2.next()
                if (configuration is None):
                    configuration = haxe_ds_StringMap()
                name1 = argXML3.att.resolve("name")
                documentation1 = StringTools.htmlUnescape(StringTools.trim(argXML3.get_innerData()))
    ...
    def printGeneralHelp():
        ...
        duell_helpers_LogHelper.info(" ")
        command = duell_objects_Arguments.commandSpecs.iterator()
        while command.hasNext():
            command1 = command.next()
            duell_helpers_LogHelper.info((((("  duell " + "\x1B[1m") + HxOverrides.stringOrNull(command1.name)) + " ") + "\x1B[0m"))
        if (duell_objects_Arguments.generalArgumentSpecs is not None):
            duell_helpers_LogHelper.info(" ")
            duell_helpers_LogHelper.info((("\x1B[4m" + "Arguments:") + "\x1B[0m"))
            arg = duell_objects_Arguments.generalArgumentSpecs.iterator()
            while arg.hasNext():
                arg1 = arg.next()
                duell_objects_Arguments.printArgument(arg1)
    ...
    def printCommandHelp():
        ...
        if duell_objects_Arguments.selectedCommand.hasPlugin:
            ...
            while key.hasNext():
                key1 = key.next()
                if StringTools.startsWith(key1,("duell" + HxOverrides.stringOrNull(duell_objects_Arguments.selectedCommand.name))):
                    duell_helpers_LogHelper.info(((((("  duell " + HxOverrides.stringOrNull(duell_objects_Arguments.selectedCommand.name)) + " ") + "\x1B[1m") + HxOverrides.stringOrNull(HxString.substr(key1,len((("duell" + HxOverrides.stringOrNull(duell_objects_Arguments.selectedCommand.name)))),None))) + "\x1B[0m"))
            duell_helpers_LogHelper.info(" ")
        ...
        if (duell_objects_Arguments.selectedCommand.arguments is not None):
            duell_helpers_LogHelper.info((("\x1B[4m" + "Arguments:") + "\x1B[0m"))
            arg = duell_objects_Arguments.selectedCommand.arguments.iterator()
            while arg.hasNext():
                arg1 = arg.next()
                duell_objects_Arguments.printArgument(arg1)
            duell_helpers_LogHelper.info(" ")
        if (duell_objects_Arguments.selectedCommand.configurationDocumentation is not None):
            ...
            doc = duell_objects_Arguments.selectedCommand.configurationDocumentation.keys()
            while doc.hasNext():
                doc1 = doc.next()
                duell_objects_Arguments.printDocumentationConfiguration(doc1,duell_objects_Arguments.selectedCommand.configurationDocumentation.h.get(doc1,None))
    ...
    def printPluginHelp():
        ...
        if (duell_objects_Arguments.pluginArgumentSpecs is not None):
            duell_helpers_LogHelper.info((("\x1B[4m" + "Arguments:") + "\x1B[0m"))
            arg = duell_objects_Arguments.pluginArgumentSpecs.iterator()
            while arg.hasNext():
                arg1 = arg.next()
                duell_objects_Arguments.printArgument(arg1)
        duell_helpers_LogHelper.info(" ")
        if (duell_objects_Arguments.pluginConfigurationDocumentation is not None):
            duell_helpers_LogHelper.info((("\x1B[4m" + "Project Configuration Documentation:") + "\x1B[0m"))
            doc = duell_objects_Arguments.pluginConfigurationDocumentation.keys()
            while doc.hasNext():
                doc1 = doc.next()
                duell_objects_Arguments.printDocumentationConfiguration(doc1,duell_objects_Arguments.pluginConfigurationDocumentation.h.get(doc1,None))
...
class duell_objects_DuellConfigJSON:
    ...
    __slots__ = ("localLibraryPath", "repoListURLs", "setupsCompleted", "lastProjectFile", "lastProjectTime", "configJSON", "configPath")
...
class duell_objects_DuellLib:
    ...
    def getDuellLib(name,version = "master",commit = ""):
        ...
        return duell_objects_DuellLib.duellLibCache.h.get(name,None).h.get(version,None)
...
class duell_objects_DuellLibReference:
    ...
    __slots__ = ("name", "gitPath", "libPath", "destinationPath")
...
class duell_objects_DuellProcess:
    ...
    def __init__(self,path,comm,args,options = None):
        ...
        self.systemCommand = (((options is not None) and ((Reflect.field(options,"systemCommand") is not None))) and Reflect.field(options,"systemCommand"))
        self.loggingPrefix = (Reflect.field(options,"loggingPrefix") if (((options is not None) and ((Reflect.field(options,"loggingPrefix") is not None)))) else "")
        self.logOnlyIfVerbose = (Reflect.field(options,"logOnlyIfVerbose") if (((options is not None) and ((Reflect.field(options,"logOnlyIfVerbose") is not None)))) else True)
        self.timeout = (Reflect.field(options,"timeout") if (((options is not None) and ((Reflect.field(options,"timeout") is not None)))) else 0.0)
        self.block = (((options is not None) and ((Reflect.field(options,"block") is not None))) and Reflect.field(options,"block"))
        self.shutdownOnError = (((options is not None) and ((Reflect.field(options,"shutdownOnError") is not None))) and Reflect.field(options,"shutdownOnError"))
        self.errorMessage = (Reflect.field(options,"errorMessage") if (((options is not None) and ((Reflect.field(options,"errorMessage") is not None)))) else "")
        self.mute = (((options is not None) and ((Reflect.field(options,"mute") is not None))) and Reflect.field(options,"mute"))
    ...
    def startStdOutListener(self):
        ...
        def _hx_local_0():
            _gthis.waitingOnStdoutMutex.acquire()
            try:
                while True:
                    _hx_str = Reflect.field(_gthis.process.p.stdout.readline(),"decode")("utf-8")
                    if ((_hx_str is None) or ((_hx_str == ""))):
                        break
                    _gthis.stdoutMutex.acquire()
                    _gthis.stdout.writeString(_hx_str)
                    _gthis.totalStdout.writeString(_hx_str)
                    _gthis.stdoutMutex.release()
                    index = (len(_hx_str) - 1)
                    if ((("" if (((index < 0) or ((index >= len(_hx_str))))) else _hx_str[index])) == "\n"):
                        _hx_str = HxString.substring(_hx_str,0,(len(_hx_str) - 1))
                        _gthis.stdoutLineBuffer.writeString(_hx_str)
                        line = _gthis.stdoutLineBuffer.getBytes().toString()
                        _gthis.log(line)
                        _gthis.lastLine = line
                        _gthis.stdoutLineBuffer = haxe_io_BytesOutput()
                    else:
                        _gthis.stdoutLineBuffer.writeString(_hx_str)
                    _gthis.timeoutTicker = True
            except Exception as _hx_e:
                _hx_e1 = _hx_e.val if isinstance(_hx_e, _HxException) else _hx_e
            ...
            tmp = _gthis.stdoutLineBuffer.getBytes().toString()
            _gthis.log(tmp)
            _gthis.finished = True
            _gthis.stdoutFinished = True
            _gthis.waitingOnStdoutMutex.release()
            _gthis.exitCodeBlocking()
    ...
    def startStdErrListener(self):
        ...
        def _hx_local_0():
            _gthis.waitingOnStderrMutex.acquire()
            try:
                while True:
                    _hx_str = Reflect.field(_gthis.process.p.stderr.readline(),"decode")("utf-8")
                    if ((_hx_str is None) or ((_hx_str == ""))):
                        break
                    _gthis.stderrMutex.acquire()
                    _gthis.stderr.writeString(_hx_str)
                    _gthis.totalStderr.writeString(_hx_str)
                    _gthis.stderrMutex.release()
                    index = (len(_hx_str) - 1)
                    if ((("" if (((index < 0) or ((index >= len(_hx_str))))) else _hx_str[index])) == "\n"):
                        _hx_str = HxString.substring(_hx_str,0,(len(_hx_str) - 1))
                        _gthis.stderrLineBuffer.writeString(_hx_str)
                        line = _gthis.stderrLineBuffer.getBytes().toString()
                        _gthis.log(line)
                        _gthis.lastLine = line
                        _gthis.stderrLineBuffer = haxe_io_BytesOutput()
                    else:
                        _gthis.stderrLineBuffer.writeString(_hx_str)
                    _gthis.timeoutTicker = True
            except Exception as _hx_e:
                ...
                if isinstance(_hx_e1, haxe_io_Eof):
                    ...
                    _this = haxe_CallStack.exceptionStack()
                    duell_helpers_LogHelper.info(("Exception with stackTrace:\n" + HxOverrides.stringOrNull("\n".join([python_Boot.toString1(x1,'') for x1 in _this]))))
            tmp = _gthis.stderrLineBuffer.getBytes().toString()
            _gthis.log(tmp)
            _gthis.finished = True
            _gthis.stderrFinished = True
            _gthis.waitingOnStderrMutex.release()
            _gthis.exitCodeBlocking()
        duell_helpers_ThreadHelper.runInAThread(_hx_local_0)
    ...
    def startTimeoutListener(self):
        ...
        if (self.timeout != 0):
            def _hx_local_0():
                while (not _gthis.finished):
                    if (not _gthis.timeoutTicker):
                        _gthis.finished = True
                        _gthis.timedout = True
                        _this = _gthis.args
                        duell_helpers_LogHelper.println((((("Process \"" + HxOverrides.stringOrNull(_gthis.command)) + " ") + HxOverrides.stringOrNull(" ".join([python_Boot.toString1(x1,'') for x1 in _this]))) + "\" timed out."))
                        _gthis.process.kill()
                        _gthis.process.close()
                        break
                    _gthis.timeoutTicker = False
                    Sys.sleep(_gthis.timeout)
                _gthis.exitCodeBlocking()
            duell_helpers_ThreadHelper.runInAThread(_hx_local_0)
    ...
    def exitCodeBlocking(self):
        ...
        if (self.exitCodeCache is None):
            ...
            if (self.shutdownOnError and ((self.timedout or ((self.exitCodeCache != 0))))):
                ...
                pathString = ((" - in path: " + HxOverrides.stringOrNull(self.path)) if ((self.path != "")) else "")
...
class duell_objects_HXCPPConfigXML:
    ...
    def getDefines(self):
        element = self.xml.get_elements()
        while element.hasNext():
            element1 = element.next()
            _g = element1.get_name()
            if (_g == "section"):
                if element1.has.resolve("id"):
                    if (element1.att.resolve("id") == "vars"):
                        return self.getDefinesFromSectionVars(element1)
    ...
    def writeDefines(self,defines):
        ...
        while key.hasNext():
            key1 = key.next()
            if ((not (key1 in env.h)) or ((env.h.get(key1,None) != defines.h.get(key1,None)))):
                definesText = (("null" if definesText is None else definesText) + HxOverrides.stringOrNull(((((("\t\t<set name=\"" + ("null" if key1 is None else key1)) + "\" value=\"") + HxOverrides.stringOrNull(duell_helpers_PathHelper.stripQuotes(defines.h.get(key1,None)))) + "\" />\n"))))
        if sys_FileSystem.exists(self.configPath):
            _hx_bytes = sys_io_File.getBytes(self.configPath)
            backup = sys_io_File.write(((HxOverrides.stringOrNull(self.configPath) + ".bak.") + Std.string(python_lib_Timeit.default_timer())),False)
    ...
    def getDefinesFromSectionVars(self,sectionElement):
        ...
        element = sectionElement.get_elements()
        while element.hasNext():
            element1 = element.next()
            _g = element1.get_name()
            if (_g == "set"):
                if element1.has.resolve("name"):
                    ...
                    if element1.has.resolve("value"):
                        value = element1.att.resolve("value")
...
class duell_objects_Haxelib:
    ...
    def isValidLibPath(self,libPath,libName):
        if sys_FileSystem.exists(libPath):
            return (libPath.find(libName) != -1)
        else:
            return False
    ...
    def selectVersion(self):
        ...
        if self.isGitVersion():
            ...
            branch = ((versionSplit[1] if 1 < len(versionSplit) else None) if ((len(versionSplit) > 1)) else "master")
            specialPath = (("/" + HxOverrides.stringOrNull((versionSplit[2] if 2 < len(versionSplit) else None))) if ((len(versionSplit) > 2)) else "")
    ...
    def isGitVersion(self):
        if (self.version is not None):
            if (not StringTools.startsWith(self.version,"ssh")):
                return StringTools.startsWith(self.version,"http")
            else:
                return True
        else:
            return False
    ...
    def getHaxelib(name,version = ""):
        ...
        return duell_objects_Haxelib.haxelibCache.h.get(name,None).h.get(version,None)
...
class duell_objects_SemVer:
    ...
    __slots__ = ("major", "minor", "patch", "plus", "rc")
...
class duell_objects_dependencies_DependencyConfigFile:
    ...
    __slots__ = ("path", "fileName", "applicationName", "duellLibs", "haxeLibs")
    ...
    def parse(self):
        ...
        try:
            ...
            while element.hasNext():
                ...
                if (_hx_local_0 == 3):
                    if (_g1 == "app"):
                        self.parseApp(element1)
                elif (_hx_local_0 == 7):
                    if (_g1 == "haxelib"):
                        self.parseHaxeLib(element1)
                elif (_hx_local_0 == 8):
                    if (_g1 == "duelllib"):
                        self.parseDuellLib(element1)
                else:
                    pass
        except Exception as _hx_e:
            _hx_e1 = _hx_e.val if isinstance(_hx_e, _HxException) else _hx_e
    ...
    def parseHaxeLib(self,e):
        name = (e.att.resolve("name") if (e.has.resolve("name")) else "")
        version = (e.att.resolve("version") if (e.has.resolve("version")) else "")
        _this = self.haxeLibs
...
class duell_objects_dependencies_DependencyLibraryObject:
    ...
    __slots__ = ("name", "configFile", "lib", "libraryDependencyObjects")
...
class duell_objects_dependencies_DotFileContentCreator:
    ...
    __slots__ = ("duellLibContent", "haxeLibsContent")
    ...
    def parseDuellLibs(self,rootNode):
        ...
        while (_g < len(subnodes)):
            ...
            _g = (_g + 1)
            lib = subnode.lib
            label = (((" [label=\"" + HxOverrides.stringOrNull(lib.version)) + "\", fontsize=10]") if ((lib is not None)) else "")
            _hx_local_1 = self
...
class duell_versioning_GitVers:
    ...
    __slots__ = ("dir", "branchList", "tagList", "currentVersion")
...
class duell_versioning_locking_LockedVersions:
    ...
    __slots__ = ("lockedVersions", "parser", "path")
    ...
    def getLastLockedVersion(self):
        ...
        while (_g < len(_g1)):
            ...
            if (version is not None):
                ...
                if ((python_lib_Time.mktime(lvTime.date.timetuple()) * 1000) > ((python_lib_Time.mktime(vTime.date.timetuple()) * 1000))):
                    version = lv
                else:
                    version = version
            else:
    ...
    def addLibraries(self,projectCommitHash,duell1,haxe1,plugins,sourcelibs):
        ...
        while (_g3 < len(sourcelibs)):
            ...
            _this = s.getPath()
            _this1 = (list(_this) if ((negativePath == "")) else _this.split(negativePath))
            relativePath = (None if ((len(_this1) == 0)) else _this1.pop())
    ...
    def getLastLockedLibraries(self):
        ...
        usedLibraries = (libraries.usedLibs if ((libraries is not None)) else haxe_ds_StringMap())
        dLibs = list()
        hLibs = list()
        plugins = list()
        sLibs = list()
        key = usedLibraries.keys()
        while key.hasNext():
            key1 = key.next()
            lockedLib = usedLibraries.h.get(key1,None)
            _g = lockedLib.type
            _g1 = _g
            _hx_local_0 = len(_g1)
            if (_hx_local_0 == 9):
    ...
    def getLastVersionDiffs(self):
        ...
        updates = (version.updates if ((version is not None)) else haxe_ds_StringMap())
        diffs = list()
        key = updates.keys()
        while key.hasNext():
            key1 = key.next()
            updates1 = updates.h.get(key1,None)
            _g = 0
            while (_g < len(updates1)):
                u = (updates1[_g] if _g >= 0 and _g < len(updates1) else None)
                _g = (_g + 1)
                x = _hx_AnonObject({'name': key1, 'type': duell_versioning_objects_LockedVersion.getLibChangeTypeAsString(u.name), 'typeReadable': duell_versioning_objects_LockedVersion.getLibChangeTypeReadable(u.name), 'oldVal': u.oldValue, 'newVal': u.newValue})
                diffs.append(x)
        return diffs
    ...
    def checkUpdates(self,oldVersion,newVersion):
        if (oldVersion is None):
            return
        newLib = newVersion.usedLibs.iterator()
        while newLib.hasNext():
            newLib1 = newLib.next()
            if (not (newLib1.name in oldVersion.usedLibs.h)):
                update = _hx_AnonObject({'name': duell_versioning_objects_LibChangeType.NEW_LIB, 'oldValue': "0.0.0", 'newValue': newLib1.version})
                newVersion.addUpdatedLib(newLib1.name,update)
                continue
            oldLib = oldVersion.usedLibs.h.get(newLib1.name,None)
            if (oldLib.version != newLib1.version):
                update1 = _hx_AnonObject({'name': duell_versioning_objects_LibChangeType.VERSION, 'oldValue': oldLib.version, 'newValue': newLib1.version})
                newVersion.addUpdatedLib(newLib1.name,update1)
            if (oldLib.commitHash != newLib1.commitHash):
                update2 = _hx_AnonObject({'name': duell_versioning_objects_LibChangeType.COMMITHASH, 'oldValue': oldLib.commitHash, 'newValue': newLib1.commitHash})
                newVersion.addUpdatedLib(newLib1.name,update2)
        oldLib1 = oldVersion.usedLibs.iterator()
        while oldLib1.hasNext():
            oldLib2 = oldLib1.next()
            if (not (oldLib2.name in newVersion.usedLibs.h)):
                update3 = _hx_AnonObject({'name': duell_versioning_objects_LibChangeType.REMOVED_LIB, 'oldValue': oldLib2.version, 'newValue': "0.0.0"})
                newVersion.addUpdatedLib(oldLib2.name,update3)
...
class duell_versioning_locking_file_LockingFileXMLParser:
    ...
    def parse(self,source):
        element = source.get_elements()
        while element.hasNext():
            element1 = element.next()
            _g = element1.get_name()
            if (_g == "build"):
                self.parseBuild(element1)

    def parseBuild(self,element):
        ts = (element.att.resolve("date") if (element.has.resolve("date")) else "0")
        commit = (element.att.resolve("commit") if (element.has.resolve("commit")) else None)
        build = duell_versioning_objects_LockedVersion(ts)
        build.projectCommitHash = commit
        child = element.get_elements()
        while child.hasNext():
            child1 = child.next()
            _g = child1.get_name()
            _g1 = _g
            ...
            if (_hx_local_0 == 4):
                if (_g1 == "libs"):
                    self.parseLibs(build,child1)
            elif (_hx_local_0 == 7):
                if (_g1 == "updates"):
                    self.parseChanges(build,child1)
            else:
                pass
        _this = self.builds
        _this.append(build)
    ...
    def parseLibs(self,currentBuild,element):
        e = element.get_elements()
        while e.hasNext():
            e1 = e.next()
            _hx_type = e1.get_name()
            name = (e1.att.resolve("name") if (e1.has.resolve("name")) else "")
            version = (e1.att.resolve("version") if (e1.has.resolve("version")) else "0.0.0")
            commitHash = (e1.att.resolve("commitHash") if (e1.has.resolve("commitHash")) else "")
    ...
    def parseChanges(self,currentBuild,element):
        l = element.get_elements()
        while l.hasNext():
            l1 = l.next()
            libname = (l1.att.resolve("name") if (l1.has.resolve("name")) else "")
            c = l1.get_elements()
            while c.hasNext():
                c1 = c.next()
                changeType = c1.get_name()
                oldValue = (c1.att.resolve("oldValue") if (c1.has.resolve("oldValue")) else "")
                newValue = (c1.att.resolve("newValue") if (c1.has.resolve("newValue")) else "")
    ...
    def getLibsContent(self,libs):
        ...
        while key.hasNext():
            key1 = key.next()
            lib = libs.h.get(key1,None)
            x = (((((((("      <" + HxOverrides.stringOrNull(lib.type)) + " name=\"") + HxOverrides.stringOrNull(lib.name)) + "\" version=\"") + HxOverrides.stringOrNull(lib.version)) + "\" ") + HxOverrides.stringOrNull((((("commitHash=\"" + HxOverrides.stringOrNull(lib.commitHash)) + "\" ") if ((lib.commitHash != "")) else "")))) + "/>")
            results.append(x)
    ...
    def getUpdatedContent(self,updates):
        ...
        while key.hasNext():
            key1 = key.next()
            changes = updates.h.get(key1,None)
            changeList = ""
            _g = 0
            ...
            results.append((((("      <update name=\"" + ("null" if key1 is None else key1)) + "\">\n") + ("null" if changeList is None else changeList)) + "      </update>"))
        results.sort(key= python_lib_Functools.cmp_to_key(self.sortStrings))
        if (len(results) == 0):
            return ""
        else:
...
class duell_versioning_objects_LockedVersion:
    ...
    __slots__ = ("ts", "projectCommitHash", "usedLibs", "updates")
    ...
    def getLibChangeType(value):
        value1 = value
        _hx_local_0 = len(value1)
        if (_hx_local_0 == 10):
            if (value1 == "commitHash"):
                return duell_versioning_objects_LibChangeType.COMMITHASH
            elif (value1 == "removedLib"):
...
class haxe_Resource:
    ...
    __slots__ = ()
    _hx_statics = ["content", "getContent", "getString", "getBytes"]
    content = None
...
class haxe_xml__Fast_AttribAccess:
    ...
    def resolve(self,name):
        ...
        if (v is None):
            ...
            if (_this.nodeType != Xml.Element):
                raise _HxException(("Bad node type, expected Element but found " + Std.string(_this.nodeType)))
            raise _HxException(((HxOverrides.stringOrNull(_this.nodeName) + " is missing attribute ") + ("null" if name is None else name)))
        return v
...
duell_defines_DuellDefines.DEFAULT_HXCPP_VERSION = "3.4.188"
duell_defines_DuellDefines.HAXE_VERSION = "3.4.4"
duell_defines_DuellDefines.ALLOWED_HAXE_VERSIONS = "3.4.4"
