...
class duell_Duell:
	...
	def setLocalJavaDistributionHome():
		...
		javaHome = Sys.getEnv("DUELL_JAVA_HOME")
...
class duell_commands_UpdateCommand:
	...
	def execute(self):
		...
		if duell_objects_Arguments.isSet("-logFile"):
			...
			self.createSchemaXml(libs.duelllibs,libs.plugins,libs.sourcelibs)
			return "success"
		self.determineAndValidateDependenciesAndDefines()
		duell_helpers_LogHelper.info(("\x1B[2m" + "------"))
	...
	def useVersionFileToRecreateSpecificVersions(self):
		...
		sLibs = lockedVersions.sourcelibs
		...
		duell_helpers_LogHelper.wrapInfo(("\x1B[2m" + "Recreating Sourcelibs"),None,"\x1B[2m")
		_g1 = 0
		while (_g1 < len(sLibs)):
			s = (sLibs[_g1] if _g1 >= 0 and _g1 < len(sLibs) else None)
			_g1 = (_g1 + 1)
			self.checkSourceLib(s)
		duell_helpers_LogHelper.wrapInfo(("\x1B[2m" + "Recreating Haxelibs"),None,"\x1B[2m")
		_g2 = 0
		while (_g2 < len(hLibs)):
			h = (hLibs[_g2] if _g2 >= 0 and _g2 < len(hLibs) else None)
			_g2 = (_g2 + 1)
		...
		while (_g3 < len(plugins)):
			p = (plugins[_g3] if _g3 >= 0 and _g3 < len(plugins) else None)
			_g3 = (_g3 + 1)
		...
		return _hx_AnonObject({'duelllibs': dLibs, 'haxelibs': hLibs, 'plugins': plugins, 'sourcelibs': sLibs})
	...
	def checkSourceLib(self,lib):
		if (not sys_FileSystem.exists(lib.getPath())):
			raise _HxException(((("Couldnt find parenting path sourcelib name: " + HxOverrides.stringOrNull(lib.name)) + " path: ") + HxOverrides.stringOrNull(lib.getPath())))
	...
	def logVersions(self):
		commit = duell_helpers_GitHelper.getCurrentCommit(self.projectDir)
		duell_versioning_locking_LockedVersionsHelper.addLockedVersion(commit,self.finalLibList.duellLibs,self.finalLibList.haxelibs,self.finalPluginList,self.finalLibList.sourceLibs)
...
class duell_helpers_GitHelper:
	...
	def setRemoteURL(path,remoteName,url):
		gitProcess = duell_helpers_GitHelper.runGitCommand(path,["remote", "set-url", remoteName, url],"setting git remote url")
	...
	def clone(gitURL,path):
		...
		gitProcess = duell_helpers_GitHelper.runGitCommand(path,["clone", gitURL, folder],"cloning git",True)
		return gitProcess.exitCode()
	...
	def pull(destination,parameters = None):
		...
		if (parameters is None):
			parameters = []
		gitProcess = duell_helpers_GitHelper.runGitCommand(destination,(["pull"] + parameters),"pulling git")
		gitProcess.blockUntilFinished()
		return gitProcess.exitCode()
	...
	def updateNeeded(destination):
		if (not duell_helpers_ConnectionHelper.isOnline()):
			return False
		duell_helpers_GitHelper.runGitCommand(destination,["remote", "update"],"checking for update on git",True)
		gitProcess = duell_helpers_GitHelper.runGitCommand(destination,["status", "-b", "--porcelain"],"checking for update on git",True)
	...
	def isRepoWithoutLocalChanges(destination):
		gitProcess = duell_helpers_GitHelper.runGitCommand(destination,["status", "-s", "--porcelain"],"checking for local changes on git",True)
		output = gitProcess.getCompleteStdout().toString()
	...
	def fetch(destination):
		if (not duell_helpers_ConnectionHelper.isOnline()):
			return
		duell_helpers_GitHelper.runGitCommand(destination,["fetch", "--tags", "--prune"],"fetching git",True)

	@staticmethod
	def getCurrentBranch(destination):
		gitProcess = duell_helpers_GitHelper.runGitCommand(destination,["rev-parse", "--verify", "--abbrev-ref", "HEAD"],"getting current branch on git",True)
		output = gitProcess.getCompleteStdout().toString()
		return HxOverrides.arrayGet(output.split("\n"), 0)

	@staticmethod
	def getCurrentCommit(destination):
		gitProcess = duell_helpers_GitHelper.runGitCommand(destination,["rev-parse", "--verify", "HEAD"],"getting current commit on git",True)
		output = gitProcess.getCompleteStdout().toString()
		return HxOverrides.arrayGet(output.split("\n"), 0)

	@staticmethod
	def getCurrentTags(destination):
		gitProcess = duell_helpers_GitHelper.runGitCommand(destination,["tag", "--contains", "HEAD"],"getting current tags on git",True)
		output = gitProcess.getCompleteStdout().toString()
		return output.split("\n")

	@staticmethod
	def getCommitForTag(destination,tag):
		gitProcess = duell_helpers_GitHelper.runGitCommand(destination,["rev-parse", (("null" if tag is None else tag) + "~0")],"getting commit for tag on git",True)
		output = gitProcess.getCompleteStdout().toString()
		return HxOverrides.arrayGet(output.split("\n"), 0)

	@staticmethod
	def listRemotes(destination):
		gitProcess = duell_helpers_GitHelper.runGitCommand(destination,["remote", "-v"],"listing remotes on git",True)
	...
	def listBranches(destination):
		gitProcess = duell_helpers_GitHelper.runGitCommand(destination,["branch", "-a"],"listing branches on git",True)
	...
	def listTags(destination):
		gitProcess = duell_helpers_GitHelper.runGitCommand(destination,["tag"],"listing tags on git",True)
		output = gitProcess.getCompleteStdout().toString()
		return output.split("\n")

	@staticmethod
	def checkoutBranch(destination,branch):
		gitProcess = duell_helpers_GitHelper.runGitCommand(destination,["checkout", branch],"checking out branch on git",True)
		return gitProcess.exitCode()

	@staticmethod
	def checkoutCommit(destination,commit):
		gitProcess = duell_helpers_GitHelper.runGitCommand(destination,["checkout", commit],"checking out commit on git",True)
		return gitProcess.exitCode()

	@staticmethod
	def createTag(destination,tagName,tagMessage):
		gitProcess = duell_helpers_GitHelper.runGitCommand(destination,["tag", "-a", tagName, "-m", tagMessage],"creating a tag on git")
		return gitProcess.exitCode()

	@staticmethod
	def pushTags(destination):
		gitProcess = duell_helpers_GitHelper.runGitCommand(destination,["push", "origin", "--tags"],"pushing a tag on git",True)
		return gitProcess.exitCode()

	@staticmethod
	def runGitCommand(path,args,errorMessage,retryOnError = False):
		if (retryOnError is None):
			retryOnError = False
		options = _hx_AnonObject({'systemCommand': True, 'loggingPrefix': "[Git]", 'block': True, 'errorMessage': errorMessage})
		if retryOnError:
...
class duell_versioning_locking_LockedVersionsHelper:
	...
	@staticmethod
	def addLockedVersion(projectCommitHash,duelllibs,haxelibs,plugins,sourcelibs):
...
class duell_versioning_locking_LockedVersions:
	...
	def addLibraries(self,projectCommitHash,duell1,haxe1,plugins,sourcelibs):
		...
		_g1 = 0
		while (_g1 < len(haxe1)):
			hLib = (haxe1[_g1] if _g1 >= 0 and _g1 < len(haxe1) else None)
			_g1 = (_g1 + 1)
		...
		_g3 = 0
		while (_g3 < len(sourcelibs)):
			s = (sourcelibs[_g3] if _g3 >= 0 and _g3 < len(sourcelibs) else None)
			_g3 = (_g3 + 1)
			negativePath = haxe_io_Path.join([duell_helpers_DuellConfigHelper.getDuellConfigFolderLocation(), "lib"])
			relativePath = None
			_this = None
			...
			if (negativePath == ""):
				...
				_this = _this1.split(negativePath)
			relativePath = (None if ((len(_this) == 0)) else _this.pop())
			lockedLib3 = _hx_AnonObject({'name': s.name, 'type': "sourcelib", 'version': relativePath, 'commitHash': ""})
			currentVersion.addUsedLib(lockedLib3)
		lastLockedVersion = self.getLastLockedVersion()
		_this2 = self.lockedVersions
		_this2.append(currentVersion)
	...
	def getLastLockedLibraries(self):
		...
		while _hx_local_1.hasNext():
			...
			if (_hx_local_0 == 9):
				if (_g == "sourcelib"):
					relativePath = lockedLib.version
					absolutePath = haxe_io_Path.join([duell_helpers_DuellConfigHelper.getDuellConfigFolderLocation(), "lib", relativePath])
					x3 = duell_objects_SourceLib(lockedLib.name, absolutePath)
					sLibs.append(x3)
			elif (_hx_local_0 == 7):
				if (_g == "haxelib"):
...
class duell_versioning_locking_file_LockingFileXMLParser:
	...
	def parseBuild(self,element):
		...
		if element.has.resolve("date"):
			...
			ts = "0"
		commit = None
		if element.has.resolve("commit"):
			commit = element.att.resolve("commit")
		else:
			commit = None
	...
	def createFileContent(self,objects):
		...
		while (_g1 < _g):
			...
			buildResult = (((("  <build date=\"" + HxOverrides.stringOrNull(b.ts)) + "\" commit=\"") + HxOverrides.stringOrNull(b.projectCommitHash)) + "\">\n")
...
class duell_versioning_objects_LockedVersion:
	...
	_hx_fields = ["ts", "projectCommitHash", "usedLibs", "updates"]
...
duell_defines_DuellDefines.ALLOWED_HAXE_VERSIONS = "3.2.0,3.2.1,3.4.0,3.4.1,3.4.2"
